...
class VolumeControlApp:
    def __init__(self):
        ...
        # 创建一个按钮并指定位置
        self.button = tk.Button(self.window,bg="white",fg="black",text="日",anchor="center",font=("黑体", 15), activebackground="red")
        self.button.place(x=25, y=25,width=50, height=50,anchor="center")
        # 创建一个滑块
                                                                                        #最大值 最小值      横还是竖          长度       宽度   是否显示当前值    刻度数量    最小移动距离       滑块的宽度     是否去除边框 
        self.scale1 = tk.Scale(self.window,font=("黑体",12),troughcolor='white',label="透",from_=100,to=0,orient="vertical",length=400,width=16,showvalue=0,tickinterval=9,resolution=10,sliderlength=50,highlightthickness=0)
        self.scale2 = tk.Scale(self.window,font=("黑体",12),troughcolor='white',label="亮",from_=100,to=0,orient="vertical",length=400,width=16,showvalue=0,tickinterval=9,resolution=10,sliderlength=50,highlightthickness=0)
        self.scale3 = tk.Scale(self.window,font=("黑体",12),troughcolor='white',label="音",from_=100,to=0,orient="vertical",length=400,width=16,showvalue=0,tickinterval=9,resolution=10,sliderlength=50,highlightthickness=0)
        ...
        self.mute_checkbutton.config(fg='black',bg="white",activebackground="white")
        ...
        self.scale3.config(fg='black',troughcolor='white',bg='white',activebackground='blue')
        self.scale2.config(fg='black',troughcolor='white',bg='white',activebackground='red')
        self.scale1.config(fg='black',troughcolor='white',bg='white',activebackground='orange')
        ...
        def 按钮事件():
            if self.window.cget("bg") == "white":
                ...
                self.button.configure(bg="black", fg="white", text="月")
                self.scale3.config(fg='white',troughcolor='black',bg='black',activebackground='blue')
                self.scale2.config(fg='white',troughcolor='black',bg='black',activebackground='red')
                self.scale1.config(fg='white',troughcolor='black',bg='black',activebackground='orange')
                self.mute_checkbutton.config(fg='white',bg="black",activebackground="black")
            else:
                self.window.configure(bg="white")
                self.button.configure(bg="white", fg="black", text="日")
                self.scale3.config(fg='black',troughcolor='white',bg='white',activebackground='blue')
                self.scale2.config(fg='black',troughcolor='white',bg='white',activebackground='red')
                self.scale1.config(fg='black',troughcolor='white',bg='white',activebackground='orange')
                self.mute_checkbutton.config(fg='black',bg="white",activebackground="white")
